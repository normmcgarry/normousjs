define([
	...
	Normous.Physics.Twod.SpringConstraint.prototype.resolve = function() {
		if(this.p1.fixed && this.p2.fixed) return;
		
		var deltaLength = this.getPositionLength();
		var diff = (deltaLength - this.restLength) / (deltaLength * (this.p1.getInverseMass() + this.p2.getInverseMass()));
		
		var dmds = this.getDelta().multiply(diff * this.stiffness);
